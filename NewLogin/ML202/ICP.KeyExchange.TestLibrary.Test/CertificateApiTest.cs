...
namespace ICP.KeyExchange.TestLibrary.Test
{
    ...
    public class CertificateApiTest
    {
        ...
        private string callNormalApi(string url, object obj, ref string decryptContent)
        {
            ...
            Console.WriteLine("VerifyOTP");
            Console.WriteLine(decryptContent);

            //// 提取Token的值
            int startIndex = decryptedData.IndexOf("\"Token\":\"") + "\"Token\":\"".Length;
            int endIndex = decryptedData.IndexOf(',', startIndex);
            string token = decryptedData.Substring(startIndex, endIndex - startIndex);
            ...
            File.WriteAllText("ntoken.txt", token);
        ...
        public void GetCellphone()
        {
           ...
           //  string url = "/app/MemberInfo/RefreshLoginToken";
            string url = "/MemberInfo/VerifyOTP";
            var request1 = new
            {
                Timestamp = DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss"),
                ...
                SMSAuthType = 1,
                //AuthCode = "114972",
                AuthCode = System.IO.File.ReadAllText(@"C:\IcashPost\NewICPLogin\ML201\ConsoleApp1\bin\Debug\AuthCode.txt"),
                Vers = "3.0.1.137",
