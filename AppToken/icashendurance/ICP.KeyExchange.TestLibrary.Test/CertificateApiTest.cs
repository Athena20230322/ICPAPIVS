...

{
    ...
    public class CertificateApiTest
    {
        ...
        private string callNormalApi(string url, object obj, ref string decryptContent)
        {
            ...
            using (StreamWriter writer1 = new StreamWriter("s.txt"))
            {
                writer1.WriteLine(s);
            }

            using (StreamWriter writer2 = new StreamWriter("a.txt"))
            {
                writer2.WriteLine(a);
            }

            using (StreamWriter writer3 = new StreamWriter("encData.txt"))
            {
                writer3.WriteLine(encData);
            }

           // string post =  s + ',' + a + ',' + encData + '\n';

            string post = s;
            string post1 = a;
            string post2 = encData;

            //post 未送出的

            using (StreamWriter writer4 = new StreamWriter("post.txt"))


            {


                writer4.WriteLine(post);

            }
            using (StreamWriter writer5 = new StreamWriter("post1.txt"))


            {


                writer5.WriteLine(post1);

            }
            using (StreamWriter writer6 = new StreamWriter("post2.txt"))


            {


                writer6.WriteLine(post2);
